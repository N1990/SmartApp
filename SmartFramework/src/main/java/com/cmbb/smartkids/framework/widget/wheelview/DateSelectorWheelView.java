...
public class DateSelectorWheelView extends RelativeLayout implements
	...
	private void initLayout(Context context) {
		LayoutInflater.from(context).inflate(R.layout.wheel_layout_date_time, this,
				true);
	...
	public void onChanged(WheelView wheel, int oldValue, int newValue) {
		String trim = null;
		int i = wheel.getId();
		if (i == R.id.wv_date_of_year) {
			trim = splitDateString(wvYear.getCurrentItemValue())
					.trim();
			tvYear.setText(trim);
			if (isLeapYear(trim)) {
				if (currentMonth == 2) {
		...
		} else if (i == R.id.wv_date_of_month) {
			trim = splitDateString(wvMonth.getCurrentItemValue())
					.trim();
		...
		} else if (i == R.id.wv_date_of_day) {
			tvDay.setText(splitDateString(wvDay.getCurrentItemValue())
					.trim());
